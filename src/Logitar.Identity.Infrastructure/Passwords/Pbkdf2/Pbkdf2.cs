...
public record Pbkdf2 : Password
{
  ...
  public Pbkdf2(string password, KeyDerivationPrf algorithm, int iterations, int saltLength, int? hashLength)
  {
  ...
  public static Pbkdf2 Decode(string value)
  {
    string[] values = value.Split(Separator);
    if (values.Length != 5 || values.First() != Key)
    {
      throw new ArgumentException($"The value '{value}' is not a valid PBKDF2 password.", nameof(value));
    }
