...
public class UniqueNameTests
{
  ...
  {
    string value = $"  {_faker.Person.UserName}  ";
    UniqueName? uniqueName = UniqueName.TryCreate(value, _uniqueNameSettings);
  ...
  {
    Assert.Null(UniqueName.TryCreate(value, _uniqueNameSettings));
  }
  ...
  {
    string value = RandomStringGenerator.GetString(999);
    var exception = Assert.Throws<FluentValidation.ValidationException>(() => UniqueName.TryCreate(value, _uniqueNameSettings));
