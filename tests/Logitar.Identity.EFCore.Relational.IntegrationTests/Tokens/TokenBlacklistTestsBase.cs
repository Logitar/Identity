...
public abstract class TokenBlacklistTestsBase : IAsyncLifetime
{
  ...
  public TokenBlacklistTestsBase()
  {
  ...
  public async Task BlacklistAsync_it_should_blacklist_identifiers_with_expiration()
  {
    ...
    foreach (string tokenId in tokenIds)
    {
      ...
      Assertions.Equal(expiresOn, entities[tokenId].ExpiresOn, TimeSpan.FromSeconds(1));
    }
  }
  ...
  public async Task PurgeAsync_it_should_remove_only_expired_items()
  {
    ...
    {
      ExpiresOn = DateTime.Now.AddDays(-1).ToUniversalTime()
    };
